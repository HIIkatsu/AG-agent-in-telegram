...
_CODE_INTENT_RE = re.compile(
    ...
    r"создай|создать|сделай|сделать|напиши|написать|перепиши|переписать|"
    r"измени|изменить|почини|починить|исправь|исправить|удали|добавь|реализуй|"
    r"запусти|запуск|тест(?:ы|ов)?|проверь(?:\s+проект)?|diff|commit|коммит|deploy|деплой|"
    r"код(?:а|е|ом)?|скрипт|терминал|команд(?:а|у|ы)|ошибк(?:а|у|и)|баг(?:а|и)?|"
    r"file|folder|repository|repo|project|create|edit|modify|fix|delete|implement|"
    r"run|tests?|check(?:\s+the\s+project)?|patch|deployment|bug|error|traceback"
    r")\b",
    re.IGNORECASE,
)
