...
_EXPLICIT_ARTIFACT_RE = re.compile(
    r"(?:"
    r"(?:(?:за|с)?генерир\w*|созд\w*|сдела\w*|нарис\w*|отправ\w*|пришл\w*|скин\w*|"
    ...
    r"|(?:(?:за|с)?генерир\w*|созд\w*|сдела\w*|нарис\w*|отправ\w*|пришл\w*|скин\w*)"
    r"\b[^\n]{0,80}?(?:картинк\w*|изображени\w*|фото\w*)"
    r"|(?:картинк\w*|изображени\w*|фото\w*)\s+(?:(?:за|с)?генерир(?:уй|овать)|"
    r"созд(?:ай|ать)|сделай|нарисуй)"
    r"|(?:(?:за|с)?генерир\w*|созд\w*|сдела\w*|напиш\w*|сверста\w*|"
    r"подготовь)\b[^\n]{0,100}?"
...
_EXPLICIT_IMAGE_RE = re.compile(
    r"(?:"
    r"(?:(?:за|с)?генерир\w*|созд\w*|сдела\w*|нарис\w*)\b[^\n]{0,80}?"
    r"(?:картинк\w*|изображени\w*|фото\w*)"
    r"|(?:картинк\w*|изображени\w*|фото\w*)\s+(?:(?:за|с)?генерир(?:уй|овать)|"
    r"созд\w*|сдела\w*|нарис\w*)"
